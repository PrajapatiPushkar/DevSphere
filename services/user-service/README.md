...
- **IDOR Protection**: Developer profile (`/api/v1/profile`), goal management (`/api/v1/goals/**`), task management (`/api/v1/tasks/**`), daily planner (`/api/v1/planner/**`), DSA progress tracking (`/api/v1/dsa/**`), developer projects (`/api/v1/projects/**`), career profile (`/api/v1/career-profile`), experience (`/api/v1/experience/**`), education (`/api/v1/education/**`), skills (`/api/v1/skills/**`), and certifications (`/api/v1/certifications/**`) endpoints enforce strict user identity scoping (`findByUserId` / `findByIdAndUserId`). Requests for non-owned entities return `404 Not Found` without leaking resource existence.
- **Product Domains**: Supports developer profiles, goals, tasks, daily planner entries, DSA progress tracking, developer project management, career profile, and structured career history collections (`Experience`, `Education`, `Skill` with case-insensitive `(user_id, name)` duplicate protection, and `Certification`).
