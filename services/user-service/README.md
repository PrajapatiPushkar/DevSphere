...
- **IDOR Protection**: Developer profile (`/api/v1/profile`), goal management (`/api/v1/goals/**`), task management (`/api/v1/tasks/**`), daily planner (`/api/v1/planner/**`), DSA progress tracking (`/api/v1/dsa/**`), developer projects (`/api/v1/projects/**`), and career profile (`/api/v1/career-profile`) endpoints enforce strict user identity scoping (`findByUserId` / `findByIdAndUserId`). Requests for non-owned tasks, goals, planner entries, DSA problems, projects, or career profiles return `404 Not Found` without leaking resource existence.
- **Goal, Task, Planner, DSA, Project & Career Domains**: Supports developer profiles, goals, tasks, daily planner entries, DSA progress tracking, developer project management, and career profiles (singleton per user `UNIQUE(user_id)`, `professionalSummary`, `currentTitle`, `targetRole`, `yearsOfExperience`, `preferredLocation`, work preference `REMOTE`, `HYBRID`, `ONSITE`, `FLEXIBLE`, availability `OPEN_TO_WORK`, `ACTIVELY_LOOKING`, `NOT_LOOKING`, `OPEN_TO_OPPORTUNITIES`, idempotent `PUT`, and user-scoped `DELETE`).
