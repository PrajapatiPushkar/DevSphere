...
- **IDOR Protection**: Developer profile (`/api/v1/profile`), goal management (`/api/v1/goals/**`), task management (`/api/v1/tasks/**`), daily planner (`/api/v1/planner/**`), and DSA progress tracking (`/api/v1/dsa/**`) endpoints enforce strict user identity scoping (`findByIdAndUserId`). Requests for non-owned tasks, goals, planner entries, or DSA problems return `404 Not Found` without leaking resource existence.
- **Goal, Task, Planner & DSA Domains**: Supports developer profiles, goals, tasks, daily planner entries, and DSA progress tracking (difficulty, topic, platform, attempts, solving time, notes, status lifecycle `TODO`, `IN_PROGRESS`, `SOLVED`, `REVISIT`, `ARCHIVED`, dynamic daily progress, and overall statistics).
