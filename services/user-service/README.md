...
- **IDOR Protection**: Developer profile (`/api/v1/profile`), goal management (`/api/v1/goals/**`), task management (`/api/v1/tasks/**`), and daily planner (`/api/v1/planner/**`) endpoints enforce strict user identity scoping (`findByIdAndUserId`). Requests for non-owned tasks, goals, or planner entries return `404 Not Found` without leaking resource existence.
- **Goal, Task & Daily Planner Domains**: Supports developer profiles, goals, tasks, and daily planner entries with calendar date scheduling, sort ordering, optional time slots, duration estimates, rescheduling, unscheduling, atomic reordering, dynamic daily summary metrics, duplicate scheduling prevention (`UNIQUE(user_id, task_id, planned_date)`), state transition guardrails, database pagination, and logical task archival (`status = ARCHIVED`).
