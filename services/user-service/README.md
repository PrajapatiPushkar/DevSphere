...
- **IDOR Protection**: Developer profile (`/api/v1/profile`), goal management (`/api/v1/goals/**`), task management (`/api/v1/tasks/**`), daily planner (`/api/v1/planner/**`), DSA progress tracking (`/api/v1/dsa/**`), and developer project management (`/api/v1/projects/**`) endpoints enforce strict user identity scoping (`findByIdAndUserId`). Requests for non-owned tasks, goals, planner entries, DSA problems, or projects return `404 Not Found` without leaking resource existence.
- **Goal, Task, Planner, DSA & Project Domains**: Supports developer profiles, goals, tasks, daily planner entries, DSA progress tracking, and developer project management (status lifecycle `PLANNED`, `IN_PROGRESS`, `COMPLETED`, `ON_HOLD`, `ARCHIVED`, project types `PERSONAL`, `COLLEGE`, `PROFESSIONAL`, `OPEN_SOURCE`, `FREELANCE`, `LEARNING`, `OTHER`, tech stack tracking, date management, URL links, server-side `completedAt` tracking, and logical archival).
